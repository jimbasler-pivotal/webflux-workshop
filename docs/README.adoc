...
Brian Clozel, Violeta Georgieva, Brian Henzelmann, Jim Basler - Pivotal
:sectanchors: true
...
The Spring WebFlux Workshop comes complete with source code, so you can work your way through with working examples. We don’t want to make assumptions on where you are coming from, so here are some helpful resources if tackling the Spring WebFlux Workshop seems just a bit too advanced.
...
For Group use `io.spring.workshop` and for Artifact use `stock-quotes`. Under Dependencies search for `Spring Reactive Web`
...
Cloud Foundry is an open source, multi cloud application platform as a service. We will use it to easily deploy our apps on Amazon Web Service. The same process may be used for Azure, GCP, vSphere, and other IaaS providers where Cloud Foundry is used. 

1. Create a free account at https://run.pivotal.io
2. Login to Pivotal Web Services
3. Download the CLI by navigating to Tools at https://console.run.pivotal.io/tools
...
Go to `https://start.spring.io` and create a Maven project with Spring Boot {spring-boot-version}. 
For Group use `io.spring.workshop` and for Artifact use `trading-service`. Under Dependencies search and add `Spring Reactive Web`, `Spring Boot DevTools`, `Thymeleaf` and `Spring Data Reactive MongoDB` to your project.
...
For brevity we are keeping these instructions simple. You may reference the finished projects in the repo if you need some help.
