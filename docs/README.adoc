...
First, let's inject our `QuoteGenerator` in the constructor for `QuoteHandler`, declare
a `Flux<Quote>` class field that emits a `Quote` every 1000 msec and can be **shared** between
multiple subscribers (look at the `Flux` operators for that).

Now create a `streamQuotes` method that streams those generated quotes 
