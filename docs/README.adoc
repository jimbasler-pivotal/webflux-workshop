...
Cloud Foundry is an open source, multi cloud application platform as a service. We will use it to easily deploy our apps on Amazon Web Service. The same process may be used for Azure, GCP, vSphere, and other IaaS providers. 
...
----
5. Update the application manifest which provides metadata to Cloud Foundry telling it how to deploy your application.
[source,bash]
...
$ cf push
----
7. Make sure your application starts. You should see something similar to this.
[source,bash]
----
...
Go to `https://start.spring.io` and create a Maven project with Spring Boot {spring-boot-version},
for Group use `io.spring.workshop` and for Artifact use `trading-service`. Under Dependencies search for and select `Spring Reactive Web`, `Spring Boot DevTools`, `Thymeleaf` and `Spring Data Reactive MongoDB`.
